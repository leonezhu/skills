...
**新一周的周一（如 2025-12-29 是周一，属于 2026-W01）：**

**绝对规则：** 新一周的周一**不参考**上一周的任何笔记，包括：
- ❌ 不读取上一周的周计划（如 2025-W52）
- ❌ 不读取上一周周日的日志（如 2025-12-28）
- ❌ 不继承上一周的未完成任务
- ❌ 不统计上一周的每周重复任务执行次数
...
**示例：2025-12-29（周一，2026-W01）**
```
❌ 错误做法：
- 读取 2025-W52 的周计划
- 读取 2025-12-28（周日）的日志
- 继承"保养摩托车"到今日

✅ 正确做法：
- 读取 2026-W01 的周计划
- 读取 Daily Plan
- 本周日志范围：2025-12-29 到 2025-12-29（仅今天，空）
- 健身房次数：本周第 1/3 次（新的一周重新开始）
```
...
# 0. 判断是否为周一（跨周边界）
今日日期 = "2025-12-29"
今日星期 = get_day_of_week(今日日期)  # "Monday"
是周一 = (今日星期 == "Monday")

# 1. 读取当前周的周计划
当前周号 = get_iso_week(今日日期)  # 例如: "2026-W01"
周计划 = read(f"Daily/{当前周号}.md")

# 2. 读取昨天日志（仅在非周一）
if not 是周一:
    昨天日期 = get_yesterday(今日日期)
    昨天 = read(f"Daily/{昨天日期}.md")
else:
    昨天 = None  # 周一不参考昨天
...
# 4. 扫描本周所有日志（用于计数）
# 关键：只扫描本周范围！
本周开始日期 = get_week_start(今日日期)  # 2025-12-29
本周结束日期 = get_week_end(今日日期)    # 2026-01-04
本周日志 = glob(f"Daily/{本周开始日期} to {本周结束日期}/*.md")
```
...
**原因：** 没有识别跨周边界，错误读取了上一周的周计划和周日日志

**修复：**
```python
# 判断是否为周一
是周一 = (get_day_of_week(今日日期) == "Monday")
...
**关键规则：**
- 周一读取：`2026-W01.md`（当前周）
- 周一不读取：`2025-W52.md`（上一周）、`2025-12-28.md`（周日）
- 健身房次数：周一永远从 1/3 开始
